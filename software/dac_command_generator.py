#!/usr/bin/env python3
"""
DAC Command Generator for FPGA CDC Communication System
...
DATA_LENGTH = 9  # Wave type (1) + Frequency word (4) + Phase word (4)

# DAC clock frequency in Hz
DAC_CLOCK_FREQ = 200_000_000  # 200MHz
...
def generate_dac_command(wave_type, frequency_hz, phase_degrees=0):
    """
    ...
    frame.append(DATA_LENGTH)  # Length low byte (9 bytes)
...
def format_command_output(command_bytes, wave_type, frequency_hz, phase_degrees):
    """
    ...
    freq_word = struct.unpack('>I', bytes(command_bytes[6:10]))[0]
    phase_word = struct.unpack('>I', bytes(command_bytes[10:14]))[0]
    
    output = []
    output.append("=" * 60)
    output.append("DAC COMMAND GENERATOR")
    output.append("=" * 60)
    ...
    for i, byte in enumerate(command_bytes):
        if i < 2:
            ...
            desc = f"Freq Word [{i-6}]"
        elif i < 14:
            desc = f"Phase Word [{i-10}]"
        else:
            desc = "Checksum"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s sine 1MHz                    # 1MHz sine wave, 0° phase
  %(prog)s triangle 500kHz -p 90        # 500kHz triangle wave, 90° phase  
  %(prog)s square 2.5MHz -p 180         # 2.5MHz square wave, 180° phase
  %(prog)s sawtooth 100kHz              # 100kHz sawtooth wave
  %(prog)s sine 1000 -o output.txt      # 1kHz sine wave, save to file
    ...
    try:
        ...
        # Generate command
        command_bytes = generate_dac_command(args.wave_type, frequency_hz, args.phase)
        ...
        if args.quiet:
            ...
            output_text = format_command_output(command_bytes, args.wave_type, frequency_hz, args.phase)
