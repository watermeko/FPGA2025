...
### SPI从机配置发送缓冲 (功能码 0x14)

该功能允许配置FPGA作为SPI从机时，主机读取的数据内容。
...
**功能说明**：
- FPGA作为SPI从机，被动响应外部主机
- 通过此命令预先配置发送缓冲区
- 当外部主机发起SPI读取时，FPGA从缓冲区输出数据

**示例**：配置从机发送 "FPGA2025"
```
发送: AA 55 14 00 08 46 50 47 41 32 30 32 35 XX
      ^^^^帧头 ^^功能码 ^^^^长度=8 ^^^^^^^^"FPGA2025"^^^^^^^^ ^^校验和
```
...
| 字段 | 字节数 | 说明 |
|------|--------|------|
| 控制字 | 1 | Bit[0]: 0=禁用上传, 1=启用上传 |

**功能说明**：
- 启用后，FPGA作为从机接收到主机写入的数据会自动上传到USB CDC
- 禁用后，从机接收数据但不上传
...
**上传数据格式**：
```
响应: AA 44 14 [高字节] [低字节] [接收数据...] [校验和]
      ^^^^上传帧头 ^^从机源 ^^^^数据长度 ^^主机写入的数据^^ ^^校验和
```
