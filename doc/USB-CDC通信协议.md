...
1、i2c的从机实现，代码已经写完了，实例化就好。从机相当于是一个掉电的寄存器
2、地址和SCL是写死的 8'h50 和 400kHz 如何从CDC发过来的指令中读到这俩个需要传输的变量
...
该功能允许用户上传任意波形数据到FPGA内部RAM，突破预定义波形限制，实现真正的任意波形输出。
...
- 波形长度：1000点
- 期望输出频率：1kHz
- 波形播放频率 = 1kHz × 1000点 = 1MHz
- sample_rate_word = round((1MHz × 2^20) / 200MHz) = 5243
- 最终发送时至少取1以避免静止输出

#### 分包传输策略

单个USB-CDC帧最大65535字节，但建议每包≤512字节数据：

**首包** (控制字[1:0] = `00`):
```
控制字 | 波形长度 | 采样率字 | 前N个采样点
  1B   |   2B     |   4B     |  剩余字节
```

**续包** (控制字[1:0] = `01`):
```
控制字 | 保留(0x0000) | 保留(0x00000000) | 后续采样点
  1B   |     2B       |       4B         |  剩余字节
```
注：续包的波形长度和采样率字段填0即可，FPGA会忽略
...
**示例1：一次性上传小波形（128点）**
```
发送: AA 55 FC 00 FF 04 00 80 01 47 AE 14 [256字节波形数据...] XX
      ^^^^帧头 ^^功能码 ^^^^长度(255) ^^控制字(写入+循环) ^^^^波形长度(128) ^^^^^^^^采样率字 ^^^^^^波形数据 ^^校验和

控制字=0x04: Bit[2]=1(循环), Bit[1:0]=00(写入新波形)
```

**示例2：分包上传大波形（2048点，分4包）**

首包（512字节数据 = 256个采样点）：
```
发送: AA 55 FC 02 03 00 08 00 01 47 AE 14 [512字节波形数据...] XX
      ^^^^帧头 ^^功能码 ^^^^长度(515) ^^控制字(写入) ^^^^波形长度(2048) ^^^^^^^^采样率字 ^^^^^^前256点 ^^校验和
...
续包2-4（每包512字节数据）：
```
发送: AA 55 FC 02 01 01 00 00 00 00 00 00 [512字节波形数据...] XX
      ^^^^帧头 ^^功能码 ^^^^长度(513) ^^控制字(追加) ^^^^填0 ^^^^^^^^填0 ^^^^^^后续点 ^^校验和

控制字=0x01: Bit[1:0]=01(追加数据)
```
...
1. **上传波形**: 发送控制字=`0x00`或`0x04`的首包，根据需要发送续包（控制字=`0x01`）
